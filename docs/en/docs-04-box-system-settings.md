...
4. In the editing form that opens, specify:
...
#### Prohibiting Identifier Binding
...
3. Configure the parameters:
    ...
    - [Access Restriction](#access-settings)
...
| Name | Description |
|-------|----------|
| **Restricted Access** | If enabled, access to the personal profile will be allowed only to users with **Administrator** service rights |
...
| Name | Parameter | Description |
|-------|----------|----------|
| **Post-Logout Redirect URI #** | `post_logout_redirect_uri` | URL to which the service will redirect the user after logging out. If no value is specified, `Redirect_uri` is used. |
...
| Name | Parameter | Description |
|-------|----------|----------|
| **Signing Algorithm for ID Token (id_token_signed_response_alg)** | `id_token_signed_response_alg` | <p> Specifies the algorithm used to sign the ID token. </p> `ID token` is a JSON Web Token (JWT) that contains claims about the user's authentication |
...
#### User Identifier Transmission Type
...
1. Go to the [Sentry.io](https://sentry.io/welcome/) website.
...
3. In the connection form that opens, specify:
