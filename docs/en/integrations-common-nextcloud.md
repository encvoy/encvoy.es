...
5. Fill in the connection parameters:
    ...
    - **Scope** - specify the required permissions for data retrieval. The mandatory scope is `openid` and the standard scope is `profile`. When specifying multiple permissions, separate them with a space. For example: `profile email openid`.
