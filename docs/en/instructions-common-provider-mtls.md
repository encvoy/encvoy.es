...
- Place server certificates (`.pem` and key) and the root CA (`ca-bundle.crt`) in a convenient directory, e.g., `certs/`.
...
1. Go to the Admin Panel → **Settings** tab.
   ...
   > 💡 To create a login method for an organization, open the **Organization Dashboard**. If the login method is needed for a specific application, open **Settings for that application**.
...
Before installation, ensure you have a certificate file in `.p12` or `.pfx` format.
